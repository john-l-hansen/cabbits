...
*   **Spatial Intent of Motion**: Transitions, pans, and zoom effects must visually align with the physical layout and coordinates of the target element. For instance, clicking a component located in the top-right (e.g., the Bookshelf) triggers a zoom transition that scales and shifts focus directly towards the top-right.
*   **Content-Isolated Transitions**: Global layout elements, such as the persistent Sidebar and Top Navigation frame, must remain static and entirely unaffected by page transition animations, fades, or entry flashes. Page-specific entry animations or flash transitions must use absolute viewport targeting (`absolute inset-0`) relative to the right-hand content area rather than screen-wide properties (`fixed inset-0`).
*   **Deferred Attention Hooks**: To prevent visual overwhelm on view entry, interactive UI elements (like Pip's greeting bubble) and focal point cues (like a pulsing beacon over the bookshelf action area) are deferred. They animate in after the main scene blur/scale entry transition has settled completely (65ms delay). The dialogue bubble uses a springy "bloop" scaling curve (`stiffness: 420, damping: 18`), while the active area displays a localized pulsing icon to naturally guide the eye.
*   **Layered Escape Navigation (Stacked Backing)**: To keep exploration fluid and friendly, deeper engagement contexts (like opening sub-location POI panels, item details cards, or overlay feedback modals) form a stacked sequence. The Escape shortcut key acts as a step-by-step backing trigger: pressing `ESC` must pop/dismiss the top-most active layer (e.g. closing an overlay dialog, then closing the card modal) rather than abruptly navigating the user multiple levels back (e.g. returning to the Home View immediately). Implement this via window capture phase keydown event listeners (`true`) that stop propagation when a localized state is active.
